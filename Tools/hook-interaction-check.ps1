# Stop-хук: прогоняет Tools/interaction-check.ps1 по изменениям рабочего дерева и блокирует ход,
# если находки новые. Набор находок хэшируется в .agent-state/InteractionCheck/.last-report — сознательно
# оставленное исключение не должно блокировать каждый ход подряд.
...
$script = Join-Path $project "Tools\interaction-check.ps1"
if (-not (Test-Path $script)) { exit 0 }

$sources = Join-Path $project "Assets\Framework"
if (-not (Test-Path $sources)) { exit 0 }

$stampDir = Join-Path $project ".agent-state\InteractionCheck"
...
# Правки файла исключений тоже считаются изменением: снятая строка обязана снова зажечь находку.
$watched = @(Get-ChildItem $sources -Recurse -Filter *.cs -File)
$exceptions = Join-Path $project "Tools\interaction-check.exceptions.txt"
if (Test-Path $exceptions) { $watched += @(Get-Item $exceptions) }
...
[Console]::Error.WriteLine("interaction-check: class interactions violate Architecture/Class-Interaction.md.`n$report")
exit 2
